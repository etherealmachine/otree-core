...
class TestCollectTicks(TestCase):
  ...
  def test_collect_ticks(self):
    ...
    start_time = datetime.datetime(2017, 2, 22, 10, 0, 0, 0,
    ...
    for i, session in enumerate(Session.objects.all()):
      ...
      for roundno in range(2):
        round_end_time = round_start_time + datetime.timedelta(seconds=10)
        ...
        curr_time = round_start_time + datetime.timedelta(milliseconds=100)
        while curr_time < round_end_time:
          ...
          d.page = 'test-page'
          d.decision = {
            d.participant.code: 0.5
          }
          d.save()
          curr_time += datetime.timedelta(
            milliseconds=random.randint(100, 1000))

        round_start_time = round_end_time + datetime.timedelta(seconds=10)
    ...
    session_query = [Q(session=session) for session in Session.objects.all()]
